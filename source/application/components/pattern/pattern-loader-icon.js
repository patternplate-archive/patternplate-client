...
function sharedStyle(animating) {
	...
	return `
		...
		.pattern-loader-icon .fill-background {
			/* runs from 5.85 to 6 */
			fill: transparent;
			animation: background-fill 6s both infinite;
			animation-play-state: ${playState};
		}
		.pattern-loader-icon .inner-horizontal {
			/* line.x1 - line.x2 */
			stroke-dasharray: 53;
			stroke-dashoffset: -53;
		...
		.pattern-loader-icon .inner-vertical {
			/* line.y1 - line.y2 */
			stroke-dasharray: 70;
			stroke-dashoffset: -70;
		...
		.pattern-loader-icon .boundary-top {
			/* line.x1 - line.x2 */
			stroke-dasharray: 53;
			stroke-dashoffset: 53;
		...
		.pattern-loader-icon .boundary-right {
			/* line.y1 - line.y2 */
			stroke-dasharray: 68;
			stroke-dashoffset: -68;
		...
		.pattern-loader-icon .boundary-bottom {
			/* line.x1 - line.x2 */
			stroke-dasharray: 53;
			stroke-dashoffset: -53;
		...
		.pattern-loader-icon .boundary-left {
			/* line.y1 - line.y2 */
			stroke-dasharray: 72;
			stroke-dashoffset: 72;
		...
		.pattern-loader-icon .p-mask {
			animation: remove-p 6s both infinite;
			animation-play-state: ${playState};
		}
		.pattern-loader-icon .logo {
			transform-origin: 51% 49%;
			animation: scale-logo 6s both infinite;
			animation-play-state: ${playState};
		}
		.pattern-loader-icon .center {
			fill: rgb(66, 165, 245);
			stroke: rgb(66, 165, 245);
			stroke: none;
			transform-origin: center;
		}
		.pattern-loader-icon .paint {
			fill: none;
			stroke: rgb(66, 165, 245);
			stroke-width: 7.5;
...
function style(inverted = false, animating = false) {
	const color = inverted ? `#fff` : `rgb(44, 44, 44)`;
	const invert = inverted ? `rgb(44, 44, 44)` : `#fff`;
	const playState = animating ? 'running' : 'paused';
	...
	return `
		...
		.pattern-loader-icon .construct,
		.pattern-loader-icon .background {
			stroke: ${color};
			stroke-width: 0.2;
		}
		.pattern-loader-icon .rough-construct {
			stroke: ${color};
			stroke-width: 0.1;
		}
		.pattern-loader-icon .background {
		...
		.pattern-loader-icon .rough-p line {
		...
		.pattern-loader-icon .p-fill path {
		...
		.pattern-loader-icon .dot {
			fill: ${color};
			stroke: none;
		}
		.pattern-loader-icon .lines {
			fill: none;
			stroke: ${color};
		}
	`;
...
export default class PatternLoaderIcon extends Component {
	static propTypes = {
	...
	render() {
		...
		return (
			<svg
				...
				width={width}
				height={height}
				className="pattern-loader-icon"
				>
				<defs>
					<style>
						{sharedStyle(animating)}
					</style>
					<style>
						{style(inverted, animating)}
					</style>
					<symbol id="perimeter">
					...
					<symbol id="p">
						<use xlinkHref="#zero-zero"/>
						<use xlinkHref="#one-zero"/>
						<use xlinkHref="#two-zero"/>
						<use xlinkHref="#zero-one"/>
						<use xlinkHref="#two-one"/>
						<use xlinkHref="#zero-two"/>
						<use xlinkHref="#one-two"/>
						<use xlinkHref="#two-two"/>
						<use xlinkHref="#zero-three"/>
					</symbol>
					<mask id="paint-mask">
						<use fill="#ffffff" xlinkHref="#perimeter"/>
						<use fill="#000000" xlinkHref="#p" className="p-mask"/>
					</mask>
				</defs>
				<g className="logo blueprint">
					<use
					...
					<path
						...
						/>
					<path className="center" d="M50.5,49.5a0.5,0.5 0 1,0 1,0a0.5,0.5 0 1,0 -1,0"/>
					<g className="rough-p">
					...
					<g className="rough-construct">
						<line className="lines diagonal-right" y2="85" x2="25" y1="16" x1="75"/>
						<line className="lines diagonal-left" y2="84" x2="76" y1="15" x1="26"/>
						<line className="lines radius" y2="49.5" x2="2" y1="49.5" x1="51"/>
					</g>
					<g className="construct">
						<line className="lines boundary-top" y2="18" x2="78" y1="18" x1="25"/>
						<line className="lines boundary-right" y2="84" x2="74" y1="16" x1="74"/>
						<line className="lines boundary-bottom" y2="81" x2="77" y1="81" x1="24"/>
						<line className="lines boundary-left" y2="86" x2="28" y1="14" x1="28"/>
						<line className="lines inner-vertical" y2="82" x2="40" y1="12" x1="40"/>
						<line className="lines inner-horizontal" y2="64" x2="77" y1="64" x1="24"/>
					</g>
				</g>
			</svg>
		);
